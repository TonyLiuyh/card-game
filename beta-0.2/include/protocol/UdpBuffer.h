...
public:
    ...
    // read pointer will be reset to the beginning of the buffer
    // write pointer will be reset to the end of the buffer
    ...
    UdpBuffer &operator<<(const boost::asio::ip::udp::endpoint &endpoint);
    ...
    UdpBuffer &operator>>(boost::asio::ip::udp::endpoint &endpoint);
    ...
    template<typename T>
    UdpBuffer &operator<<(const std::multiset<T> &vec) {
        *this << (uint32_t) vec.size();
        for (auto iter = vec.begin(); iter != vec.end(); ++iter) {
            *this << *iter;
        }
    ...
    template<typename T>
    UdpBuffer &operator>>(std::multiset<T> &set) {
        std::multiset<T> temp(set);
        uint32_t size;
        *this >> size;
        for (size_t i = 0; i < size; ++i) {
            T value;
