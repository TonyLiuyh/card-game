...
const int Canvas::RED = 1;
const int Canvas::GREEN = 2;
const int Canvas::BLUE = 3;
const int Canvas::YELLOW = 4;
const int Canvas::GREY = 5;
...
void Canvas::AddComponent(const UIComponent::ptr &component) {
    components_not_in_grid.emplace(component);
}
...
void Canvas::RemoveComponent(const UIComponent::ptr &component) {
    ...
    if (iter != components.end()) {
        ...
        if (components_not_in_grid.find(component) != components_not_in_grid.end()) {
            components_not_in_grid.erase(component);
        }
    }
}
...
void Canvas::Show() {
    ...
    for (const auto & component : components_not_in_grid) {
        if (component->Active()) {
            component->Draw();
        }
    }
...
void Canvas::Up() {
    if (current_component == nullptr || !current_component->Active()) {
        if (current_component != nullptr) {
            current_component->Unselect();
        }
